...
def load_config(file_path='data/config.yml'):
    try:
...
def get_local_timezone():
    """Get local timezone from TZ environment variable or default to UTC"""
...
def find_new_season_shows(sonarr_url, api_key, all_series, tag_mapping, future_days_new_season, skip_unmonitored=False):
    local_tz = get_local_timezone()
    cutoff_date = datetime.now(local_tz) + timedelta(days=future_days_new_season)
    ...
    for series in all_series:
        ...
        for ep in episodes:
            ...
            air_date = convert_utc_to_local(air_date_str)
...
def find_upcoming_regular_episodes(sonarr_url, api_key, all_series, future_days_upcoming_episode, skip_unmonitored=False, ignore_finales_tags=None, tag_mapping=None):
    """Find shows with upcoming non-premiere, non-finale episodes within the specified days"""
    local_tz = get_local_timezone()
    cutoff_date = datetime.now(local_tz) + timedelta(days=future_days_upcoming_episode)
    now_local = datetime.now(local_tz)
    matched_shows = []
    skipped_shows = []
...
def find_upcoming_finales(sonarr_url, api_key, all_series, future_days_upcoming_finale, skip_unmonitored=False, ignore_finales_tags=None, tag_mapping=None):
    """Find shows with upcoming season finales within the specified days"""
    local_tz = get_local_timezone()
    cutoff_date = datetime.now(local_tz) + timedelta(days=future_days_upcoming_finale)
    matched_shows = []
    skipped_shows = []
...
def find_recent_season_finales(sonarr_url, api_key, all_series, recent_days_season_finale, skip_unmonitored=False, ignore_finales_tags=None, tag_mapping=None):
...
def find_recent_final_episodes(sonarr_url, api_key, all_series, recent_days_final_episode, skip_unmonitored=False, ignore_finales_tags=None, tag_mapping=None):
...
def find_new_season_started(sonarr_url, api_key, all_series, recent_days_new_season_started, skip_unmonitored=False):
        ...
        if enable_text and all_tvdb_ids:
            ...
            # For NEW_SEASON and UPCOMING_FINALE with [#] placeholder (with dates)
                ...
                for date_str in sorted(date_season_to_tvdb_ids):
                    formatted_date = format_date(date_str, date_format, capitalize_dates, simplify_next_week)
...
def main():
    ...
    try:
        # Get Sonarr configuration from environment variables
        sonarr_url = os.getenv('SONARR_URL')
        sonarr_api_key = os.getenv('SONARR_API_KEY')
        sonarr_timeout = int(os.getenv('SONARR_TIMEOUT', '90'))
        
        if not sonarr_url:
            print(f"{RED}ERROR: SONARR_URL environment variable is required{RESET}")
            sys.exit(1)
        if not sonarr_api_key:
            print(f"{RED}ERROR: SONARR_API_KEY environment variable is required{RESET}")
            sys.exit(1)
        
        # Process and validate Sonarr URL
        sonarr_url = process_sonarr_url(sonarr_url, sonarr_api_key, sonarr_timeout)
            ...
            for filename in files_to_delete:
                ...
                try:
                    if os.path.exists(file_path):
                        os.remove(file_path)
                        debug_print(f"{GREEN}Deleted: {file_path}{RESET}", config)
                except Exception as e:
                    print(f"{ORANGE}Warning: Could not delete {file_path}: {str(e)}{RESET}")
        ...
        if process_new_season_soon:
            matched_shows, skipped_shows = find_new_season_shows(
                sonarr_url, sonarr_api_key, all_series, tag_mapping, future_days_new_season, skip_unmonitored
            )
        ...
        if process_new_season_started:
            new_season_started_shows = find_new_season_started(
                sonarr_url, sonarr_api_key, all_series, recent_days_new_season_started, skip_unmonitored
            )
        ...
        if process_upcoming_episode:
            upcoming_eps, skipped_eps = find_upcoming_regular_episodes(
                sonarr_url, sonarr_api_key, all_series, future_days_upcoming_episode, skip_unmonitored, ignore_finales_tags, tag_mapping
            )
        ...
        if process_upcoming_finale:
            finale_eps, skipped_finales = find_upcoming_finales(
                sonarr_url, sonarr_api_key, all_series, future_days_upcoming_finale, skip_unmonitored, ignore_finales_tags, tag_mapping
            )
        ...
        if process_season_finale:
            season_finale_shows = find_recent_season_finales(
                sonarr_url, sonarr_api_key, all_series, recent_days_season_finale, skip_unmonitored, ignore_finales_tags, tag_mapping
            )
        ...
        if process_final_episode:
            final_episode_shows = find_recent_final_episodes(
                sonarr_url, sonarr_api_key, all_series, recent_days_final_episode, skip_unmonitored, ignore_finales_tags, tag_mapping
            )
